...
if(dataset_id == "bird" & model_id != "SSDM"){
  ...
  for(i in seq(1, 1000, 10)){
    ...
    command <- sprintf("system('sbatch -p cloud,physical --cpus-per-task=1 --mem=51200 --time=30-00 --job-name=B_%1$s_%2$s_%3$s --output=B_%1$s_%2$s_%3$s.out scripts/slurm/bird_submission.slurm %1$s 3 %2$s %3$s %4$s')",
                       model_command_arg,
                       fold_id,
                       start_sample,
                       end_sample)
